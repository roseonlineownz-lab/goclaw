...
CREATE INDEX IF NOT EXISTS idx_tt_tsv                  ON team_tasks USING gin(tsv);
CREATE INDEX IF NOT EXISTS idx_tt_embedding            ON team_tasks USING hnsw(embedding halfvec_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_project_grants_project_user_unique
    ON project_grants(project_id, user_id)
    WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_project_grants_project_team_unique
    ON project_grants(project_id, team_id)
...
CREATE INDEX IF NOT EXISTS idx_mem_embedding    ON memory_chunks USING hnsw(embedding halfvec_cosine_ops) WITH (m = 16, ef_construction = 64);
CREATE INDEX IF NOT EXISTS idx_mem_tsv          ON memory_chunks USING gin(tsv);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_kg_entities_unique ON kg_entities(agent_id, COALESCE(user_id::text,''), external_id);
...
CREATE INDEX IF NOT EXISTS idx_kg_embedding        ON kg_entities USING hnsw(embedding halfvec_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_kg_relations_unique ON kg_relations(agent_id, COALESCE(user_id::text,''), source_entity_id, relation_type, target_entity_id);
...
CREATE INDEX IF NOT EXISTS idx_vault_docs_embedding    ON vault_documents USING hnsw(embedding halfvec_cosine_ops) WITH (m = 16, ef_construction = 64);
CREATE INDEX IF NOT EXISTS idx_vault_docs_delegation   ON vault_documents((metadata->>'delegation_id'))
...
CREATE INDEX IF NOT EXISTS idx_vault_versions_doc ON vault_versions(doc_id);
...
CREATE INDEX IF NOT EXISTS idx_skills_pinned            ON skills(pinned)      WHERE pinned = TRUE;
CREATE INDEX IF NOT EXISTS idx_skills_embedding         ON skills USING hnsw(embedding halfvec_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE INDEX IF NOT EXISTS idx_heartbeats_due ON agent_heartbeats(next_run_at) WHERE enabled = TRUE AND next_run_at IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_hooks_lookup ON hooks(event) WHERE enabled = TRUE;
