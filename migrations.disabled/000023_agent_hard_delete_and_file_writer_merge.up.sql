...
-- Migrate legacy group_file_writers → agent_config_permissions when upgrading
-- from the old split schema. Fresh/squashed installs do not have this table.
DO $$
BEGIN
  IF to_regclass('public.group_file_writers') IS NOT NULL THEN
...
DROP TABLE IF EXISTS group_file_writers;
...
ALTER TABLE agent_sessions DROP CONSTRAINT IF EXISTS sessions_agent_id_fkey;
ALTER TABLE agent_sessions ADD CONSTRAINT sessions_agent_id_fkey
    FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE CASCADE;

ALTER TABLE cron_jobs DROP CONSTRAINT IF EXISTS cron_jobs_agent_id_fkey;
ALTER TABLE cron_jobs ADD CONSTRAINT cron_jobs_agent_id_fkey
    FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE CASCADE;

ALTER TABLE heartbeat_run_logs DROP CONSTRAINT IF EXISTS heartbeat_run_logs_agent_id_fkey;
ALTER TABLE heartbeat_run_logs ADD CONSTRAINT heartbeat_run_logs_agent_id_fkey
    FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE CASCADE;

ALTER TABLE agent_teams DROP CONSTRAINT IF EXISTS agent_teams_lead_agent_id_fkey;
...
ALTER TABLE team_messages DROP CONSTRAINT IF EXISTS team_messages_from_agent_id_fkey;
ALTER TABLE team_messages ADD CONSTRAINT team_messages_from_agent_id_fkey
    FOREIGN KEY (from_agent_id) REFERENCES agents(id) ON DELETE CASCADE;

ALTER TABLE delegation_history DROP CONSTRAINT IF EXISTS delegation_history_source_agent_id_fkey;
...
ALTER TABLE delegation_history DROP CONSTRAINT IF EXISTS delegation_history_target_agent_id_fkey;
...
ALTER TABLE team_workspace_files DROP CONSTRAINT IF EXISTS team_workspace_files_uploaded_by_fkey;
...
ALTER TABLE team_workspace_file_versions DROP CONSTRAINT IF EXISTS team_workspace_file_versions_uploaded_by_fkey;
ALTER TABLE team_workspace_file_versions ADD CONSTRAINT team_workspace_file_versions_uploaded_by_fkey
    FOREIGN KEY (uploaded_by) REFERENCES agents(id) ON DELETE CASCADE;

ALTER TABLE team_workspace_comments DROP CONSTRAINT IF EXISTS team_workspace_comments_agent_id_fkey;
...
ALTER TABLE cron_run_logs DROP CONSTRAINT IF EXISTS cron_run_logs_agent_id_fkey;
ALTER TABLE cron_run_logs ADD CONSTRAINT cron_run_logs_agent_id_fkey
    FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE SET NULL;

ALTER TABLE team_tasks DROP CONSTRAINT IF EXISTS team_tasks_owner_agent_id_fkey;
...
ALTER TABLE team_tasks DROP CONSTRAINT IF EXISTS team_tasks_created_by_agent_id_fkey;
...
ALTER TABLE team_messages DROP CONSTRAINT IF EXISTS team_messages_to_agent_id_fkey;
...
ALTER TABLE team_task_comments DROP CONSTRAINT IF EXISTS team_task_comments_agent_id_fkey;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'team_task_attachments'
      AND column_name = 'created_by_agent_id'
  ) THEN
    EXECUTE 'ALTER TABLE team_task_attachments DROP CONSTRAINT IF EXISTS team_task_attachments_added_by_fkey';
    EXECUTE 'ALTER TABLE team_task_attachments DROP CONSTRAINT IF EXISTS team_task_attachments_created_by_agent_id_fkey';
    EXECUTE 'ALTER TABLE team_task_attachments ADD CONSTRAINT team_task_attachments_created_by_agent_id_fkey FOREIGN KEY (created_by_agent_id) REFERENCES agents(id) ON DELETE SET NULL';
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'team_task_attachments'
    ...
    EXECUTE 'ALTER TABLE team_task_attachments DROP CONSTRAINT IF EXISTS team_task_attachments_added_by_fkey';
    EXECUTE 'ALTER TABLE team_task_attachments ADD CONSTRAINT team_task_attachments_added_by_fkey FOREIGN KEY (added_by) REFERENCES agents(id) ON DELETE SET NULL';
  END IF;
END $$;
